...
const provider = new ethers.providers.JsonRpcProvider("http://127.0.0.1:8545");  // command for running on anvil
// const provider = new ethers.providers.JsonRpcProvider(API_URL);
const signer = new ethers.Wallet(PRIVATE_KEY, provider);
const {abi} = require("./out/productAPI.sol/productAPI.json");
...
app.post('/products', async(req, res) => {
    try {
        ...
        const tx = await contractInstance.createProduct(id, name, price, quantity);
        await tx.wait();
